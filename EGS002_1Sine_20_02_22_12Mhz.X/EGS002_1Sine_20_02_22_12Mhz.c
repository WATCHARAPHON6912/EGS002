...
0,18,18,18,18,18,27,0,  		 //      0     á
0,14,17,29,21,21,25,0,  		 //      1     ´
0,0,4,4,4,4,6,0,       			 //      2     à
0,2,31,1,29,19,25,0,             //      3     Ê
1,15,6,9,14,1,6,0,     			 //      4     ÃÕ
24,24,7,8,8,8,7,0,     			 //      5     Í§ÈÒC
0,0,20,23,29,22,23,0,            //      6     Hz
0,0,9,21,18,0,0,0}; 			 //      7     ÃÙ»«ÒÂàÇ¿   
//==============================================================================
const unsigned char character2[64] = {
0,0,25,9,21,21,10,0,             // 	  0	¾   
26,12,7,8,15,1,14,0,  			 //      1     Ãé
0,0,14,1,13,9,14,0,  			 //      2     Í
0,0,25,9,9,25,30,0,       		 //      3     Á
1,15,6,9,14,1,6,0,     			 //      4     ÃÕ
24,24,7,8,8,8,7,0,     			 //      5     Í§ÈÒC
0,0,20,23,29,22,23,0,            //      6     Hz
0,0,9,21,18,0,0,0};  			 //      7     ÃÙ»«ÒÂàÇ¿   
...
//****************************************************************************** 
    ...
    lcd_xy_char(1, 1, 0);        //¾
    lcd_xy_char(1, 2, 1);     	 //Ãé
    lcd_xy_char(1, 3, 2);  	 //Í
    lcd_xy_char(1, 4, 3);       //Á
    lcd_write_str(" 50");
    lcd_write_char(6);
    sbi(PORTB,5);  //************** ON AC OUTPUT Relay àÁ×èÍ¾ÃéÍÁáÅéÇ 
//==============================================================================
    while(1){ 
       uint8_t i;
       float average;
 ...
 //============================================================================== 
      V_Read = read_adc(VLAVEL);
      Volts_Show = V_Read >> 3;      // àÍÒá¤èäÁèà¡Ô¹ 127
      N_Volts_Show = COMPENSATE - Volts_Show;       //à¾×èÍª´àªÂ¡ÒÃ¤Çº¤ØÁâÇÅµìä¿ AC ¢ÒÍÍ¡ AC Adj.
...
ISR(TIMER2_OVF_vect) {
    ...
    switch (DUTY_CYCLE){
    case 0:
      cbi(PORTB,1);   //digitalWrite(Sink1, LOW); //ÊÕà¢ÕÂÇ ã¹Êâ¤»
    break;
    case 1:  
      sbi (PORTB,2);  //digitalWrite(Sink2, HIGH);  //ÊÕá´§ ã¹Êâ¤»
    break;
    case 65:
      cbi (PORTB,2);  //digitalWrite(Sink2, LOW); //ÊÕá´§ ã¹Êâ¤»
    break;
    case 66:  
      sbi (PORTB,1);  //digitalWrite(Sink1, HIGH);  //ÊÕà¢ÕÂÇ ã¹Êâ¤»
    break;
   }
